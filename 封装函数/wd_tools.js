...
// 默认时间：500
// 运动时间：easeOut
wd.animate = function (obj, attrs, duration, fx, fn){
...
//碰撞检测  方形
wd.duang = function(current,target){
...
//抖函数
wd.shake = function (obj, attr, nums, fn){
  if(obj.shake) return;
  var arr = [], _this = this;
...
wd.css = function (){
  ...
  if(len == 2){
    if(typeof attr === 'string'){
      ...
      return getComputedStyle(obj)[attr]*1;
    }else{
      for(var key in attr){
  ...
  function getStyle(attr){
    return attr == 'width' || attr == 'height' || attr == 'left' || attr == 'bottom' || attr == 'right' || attr == 'margin-left' || attr == 'marginLeft' || attr == 'margin-right' || attr == 'marginRight' || attr == 'margin-bottom' || attr == 'marginBottom' || attr == 'margin-top' || attr == 'marginTop' || attr == 'top';
  }
...
wd.setTransform = function (obj, attr, val){
...
wd.Tween = {
	...
	bounceIn: function(t, b, c, d){    //弹球减振（弹球渐出）
		return c - wd.Tween['bounceOut'](d-t, 0, c, d) + b;
	},
	...
	bounceBoth: function(t, b, c, d){
		if (t < d/2) {
			return wd.Tween['bounceIn'](t*2, 0, c, d) * 0.5 + b;
		}
		return wd.Tween['bounceOut'](t*2-d, 0, c, d) * 0.5 + c*0.5 + b;
	}
}
//鼠标滚轮判断兼容所有浏览器
